...
public interface KnowledgeGraphRepository extends JpaRepository<KnowledgeGraph, Long> {
    ...
    boolean existsByOwnerAndTitleIgnoreCase(SystemAccount owner, String title);

    boolean existsByOwner_IdAndTitleIgnoreCase(Long ownerId, String title);

    @Query("SELECT COUNT(kg) > 0 FROM KnowledgeGraph kg WHERE kg.owner.id = :ownerId AND LOWER(kg.title) = LOWER(:title)")
    boolean existsByOwnerIdAndTitleIgnoreCase(@Param("ownerId") Long ownerId, @Param("title") String title);
